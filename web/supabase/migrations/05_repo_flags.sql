-- 05_repo_flags.sql
--
-- Phase 4: simple, global curation (no login required).
-- Lets you "save" (bookmark) gems and "hide" noise. Flags are keyed by the stable
-- github_id so they survive the collector's archive-and-clear cycle.
--
-- This migration also redefines get_repo_stats so the saved/hidden state is returned
-- per row and the "view" filter (default / saved / hidden / all) is applied
-- server-side, which keeps pagination correct.
...
-- This is a personal, single-user tool with no auth, so flags are global and
-- writable by the anon role. (Wrap with auth.uid() later if multi-user is needed.)
ALTER TABLE repo_flags ENABLE ROW LEVEL SECURITY;

DROP POLICY IF EXISTS "Allow public read access on repo_flags" ON repo_flags;
CREATE POLICY "Allow public read access on repo_flags"
    ON repo_flags FOR SELECT TO public USING (true);

DROP POLICY IF EXISTS "Allow public insert on repo_flags" ON repo_flags;
CREATE POLICY "Allow public insert on repo_flags"
    ON repo_flags FOR INSERT TO public WITH CHECK (true);

DROP POLICY IF EXISTS "Allow public delete on repo_flags" ON repo_flags;
CREATE POLICY "Allow public delete on repo_flags"
    ON repo_flags FOR DELETE TO public USING (true);

-- Redefine get_repo_stats with saved/hidden awareness + star-range filtering.
-- Drop every prior overload so we don't leave an ambiguous duplicate behind.
DROP FUNCTION IF EXISTS get_repo_stats(INT, TEXT, INT, INT, FLOAT, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_repo_stats(INT, TEXT, INT, INT, FLOAT, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS get_repo_stats(INT, TEXT, INT, INT, FLOAT, TEXT, TEXT, TEXT, INT, INT);
